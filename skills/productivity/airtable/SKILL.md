...
4. Store the token in `~/.maia/.env` (or via `hermes setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.maia/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
