...
This guide walks you through connecting Coorporate Hermes to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- Coorporate Hermes installed and running (`hermes gateway`)
