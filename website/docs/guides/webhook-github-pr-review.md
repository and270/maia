...
This guide walks you through connecting Maia to GitHub so it automatically fetches a pull request's diff, analyzes the code changes, and posts a comment — triggered by a webhook event with no manual prompting.
...
- Maia installed and running (`hermes gateway`)
...
Add the following to your `~/.maia/config.yaml`:
...
```bash
tail -f "${MAIA_HOME:-$HOME/.hermes}/logs/gateway.log"
```
