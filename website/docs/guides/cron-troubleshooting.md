...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.maia/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.maia/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.maia/.env` |
...
| `local` | Write access to `~/.maia/cron/output/` |
...
2. `~/.maia/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
...
```bash
ls ~/.maia/scripts/your-script.py   # Must exist
hermes cron edit <job_id> --script ~/.maia/scripts/your-script.py
```
...
Jobs are stored in `~/.maia/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.maia/cron/jobs.json
chmod 600 ~/.maia/cron/jobs.json   # Your user should own it
```
...
2. Check `~/.maia/logs/agent.log` for scheduler messages and `~/.maia/logs/errors.log` for warnings
3. Open an issue at [github.com/and270/maia](https://github.com/and270/maia) with:
