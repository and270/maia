...
What happens:
1. The skill directory is copied to `~/.maia/skills/`
...
```bash
mkdir -p ~/.maia/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.maia/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
The skill appears automatically — no registration needed. Drop it in `~/.maia/skills/` and it's live.
...
**Use categories.** Organize skills into subdirectories (`~/.maia/skills/devops/`, `~/.maia/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.
