...
This tutorial walks you through setting up a Telegram bot powered by Maia that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Maia installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.maia/.env`
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Maia. DM me for help with code, research, debugging, and more.
   ```
...
**Option 2:** Set it manually in `~/.maia/.env`:
...
Control how much detail the bot shows when using tools. In `~/.maia/config.yaml`:
...
Customize how the bot communicates by editing `~/.maia/SOUL.md`:
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.maia/logs/gateway.log` (macOS) |
| Cron job output | `~/.maia/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.maia/cron/jobs.json` |
| Pairing data | `~/.maia/pairing/` |
| Session history | `~/.maia/sessions/` |
