...
Jobs are stored in `~/.maia/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.maia/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.maia/cron/output/` |
