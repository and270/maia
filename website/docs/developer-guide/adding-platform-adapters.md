...
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.maia/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
Most users set up a platform by dropping env vars into `~/.maia/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `hermes gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Maia.
author: Your Name
requires_env:
