...
|--------|-----------------|
| `~/.maia/.env` | API keys, bot tokens, platform credentials |
| `~/.maia/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.maia/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- PID file at `~/.maia/gateway.pid` — profile-scoped process tracking
