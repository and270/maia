...
Maia uses a SQLite database (`~/.maia/state.db`) to persist session
...
db = SessionDB()                           # Default: ~/.maia/state.db
...
Default path: `~/.maia/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.maia/` by default, or the value of `MAIA_HOME` environment variable.
