...
sidebar_position: 1
title: "Architecture"
description: "Maia internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Maia internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── hermes_constants.py       # MAIA_HOME, profile-aware paths
...
| **Profile isolation** | Each profile (`hermes -p <name>`) gets its own MAIA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
