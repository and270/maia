...
description: "How to build an image-generation backend plugin for Maia"
---
...
2. **User** — `~/.maia/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
...
A bundled plugin is complete at this point. User plugins at `~/.maia/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hermes plugins enable <name>`).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $MAIA_HOME/cache/images/ via save_b64_image()
...
echo "image_gen:" >> $MAIA_HOME/config.yaml
echo "  provider: my-backend" >> $MAIA_HOME/config.yaml
