...
description: "How to build a memory provider plugin for Maia"
---

# Building a Memory Provider Plugin

Memory provider plugins give Maia persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active MAIA_HOME path. Use for storage.
        """
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.maia`:
...
data_dir = Path("~/.maia/my-provider").expanduser()
```
