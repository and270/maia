...
| `--ignore-user-config` | Ignore `~/.maia/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
...
| `hermes checkpoints` | Inspect / prune / clear `~/.maia/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
...
| `hermes plugins` | Manage Maia plugins (install, enable, disable, remove). |
| `hermes tools` | Configure enabled tools per platform. |
| `hermes sessions` | Browse, export, prune, rename, and delete sessions. |
| `hermes insights` | Show token/cost/activity analytics. |
| `hermes fallback` | Interactive manager for the fallback provider chain. |
| `hermes claw` | OpenClaw migration helpers. |
| `maia dashboard` | Launch the web dashboard for managing config, API keys, sessions, and governed admin workflows. |
| `hermes profile` | Manage profiles — multiple isolated Hermes instances. |
| `hermes completion` | Print shell completion scripts (bash/zsh/fish). |
| `hermes version` | Show version information. |
| `hermes update` | Pull latest code and reinstall dependencies. `--check` prints commit diff without pulling; `--backup` takes a pre-pull `MAIA_HOME` snapshot. |
...
| `--ignore-user-config` | Ignore `~/.maia/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
...
Per-run overrides (no mutation to `~/.maia/config.yaml`):
...
| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `MAIA_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `hermes update`. |
...
hermes slack manifest --write      # write to ~/.maia/slack-manifest.json
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$MAIA_HOME/slack-manifest.json`. |
...
Multi-profile, multi-project collaboration board. Each install can host many boards (one per project, repo, or domain); each board is a standalone queue with its own SQLite DB and dispatcher scope. New installs start with one board called `default`, whose DB is `~/.maia/kanban.db` for back-compat; additional boards live at `~/.maia/kanban/boards/<slug>/kanban.db`. The gateway-embedded dispatcher sweeps every board per tick.
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.maia/kanban/current`). |
...
Board resolution order (highest precedence first): `--board <slug>` flag → `HERMES_KANBAN_BOARD` env var → `~/.maia/kanban/current` file → `default`.
...
Subscriptions persist to `~/.maia/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.
...
| **Identity** | Active profile name, MAIA_HOME path |
...
Inspect and manage the shadow git store at `~/.maia/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.
...
View, tail, and filter Hermes log files. All logs are stored in `~/.maia/logs/` (or `<profile>/logs/` for non-default profiles).
...
| `backup` | Take a manual tar.gz snapshot of `~/.maia/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.maia/skills/` from a snapshot (defaults to newest) |
...
Inspect shell-script hooks declared in `~/.maia/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.maia/shell-hooks-allowlist.json`.
...
Migrate your OpenClaw setup to Hermes. Reads from `~/.openclaw` (or a custom path) and writes to `~/.maia`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.maia/` (by default a single restore-point archive is written to `~/.maia/backups/pre-migration-*.zip` before apply; restorable with `hermes import`). |
...
```bash
maia dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. Requires `pip install maia[web]` (FastAPI + Uvicorn). See [Web Dashboard](/docs/user-guide/features/web-dashboard) for full documentation.
...
maia dashboard --port 8080 --no-open
```
...
maia -p work chat -q "Hello from work profile"
```
...
| `--backup` | Create a labeled pre-update snapshot of `MAIA_HOME` (config, auth, sessions, skills, pairing data) before pulling. Default is **off** — the previous always-backup behavior was adding minutes to every update on large homes. Flip it on permanently via `update.backup: true` in `config.yaml`. |
...
- **Pairing data snapshot.** Even when `--backup` is off, `hermes update` takes a lightweight snapshot of `~/.maia/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `hermes backup restore --state pre-update` if a pull rewrites a file you were editing.
