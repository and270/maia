...
Maia is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
The approval system supports three modes, configured via `approvals.mode` in `~/.maia/config.yaml`:
...
Configure the timeout in `~/.maia/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.maia/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.maia/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.maia/config.yaml`:
...
Set allowed user IDs as comma-separated values in `~/.maia/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.maia/.env to allow open access,
...
Control how unauthorized direct messages are handled in `~/.maia/config.yaml`:
...
**Storage:** Pairing data is stored in `~/.maia/pairing/` with per-platform JSON files:
...
Container resources are configurable in `~/.maia/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.maia/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `MAIA_HOME`. Skills declare these in frontmatter:
...
When loaded, Hermes checks if these files exist in the active profile's `MAIA_HOME` and registers them for mounting:
...
Paths are relative to `~/.maia/`. Files are mounted to `/root/.hermes/` inside the container.
...
4. **Store secrets securely** — keep API keys in `~/.maia/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.maia/logs/` for unauthorized access attempts
