...
1. loads jobs from `~/.maia/cron/jobs.json`
...
A file lock at `~/.maia/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.maia/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.maia/cron/output/`), but no message is sent to the delivery target.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.maia/scripts/` (same sandboxing rule as the pre-run script gate).
...
Hermes will write the check script to `~/.maia/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.maia/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.maia/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.maia/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.maia/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Hermes reads Job 1's most recent output from `~/.maia/cron/output/{job1_id}/*.md`
...
Jobs are stored in `~/.maia/cron/jobs.json`. Output from job runs is saved to `~/.maia/cron/output/{job_id}/{timestamp}.md`.
