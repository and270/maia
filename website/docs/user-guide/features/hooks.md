...
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.maia/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.maia/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.maia/hooks/` containing two files:
...
```yaml
# ~/.maia/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.maia/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.maia/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.maia/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path
...
```yaml
# ~/.maia/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.maia/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.maia/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."
...
1. A file at `~/.maia/BOOT.md` with natural-language startup instructions.
...
Create `~/.maia/BOOT.md`. Write it as if you were giving instructions to a human assistant:
...
**`~/.maia/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.maia/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.maia/hooks/boot-md/handler.py`**

```python
"""Run ~/.maia/BOOT.md on every gateway startup."""
...
Delete `~/.maia/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.maia/hooks/`
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.maia/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.maia/agent-hooks/` (by convention) | `~/.maia/plugins/<name>/` | `~/.maia/hooks/<name>/` |
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.maia/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.maia/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.maia/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.maia/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.maia/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.maia/agent-hooks/inject-cwd-context.sh
...
hooks:
  subagent_stop:
    - command: "~/.maia/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.maia/agent-hooks/log-orchestration.sh
log=~/.maia/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Hermes sees it, then persists the decision to `~/.maia/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.
...
- Keep scripts inside `~/.maia/agent-hooks/` so the path is easy to audit.
