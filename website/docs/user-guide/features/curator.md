...
It exists so that skills created via the [self-improvement loop](/docs/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.maia/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

The curator **never touches** bundled skills (shipped with the repo) or hub-installed skills (from [agentskills.io](https://agentskills.io)). It only reviews skills the agent itself authored. It also **never auto-deletes** — the worst outcome is archival into `~/.maia/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/and270/maia/issues/7816).
...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.maia/skills/.archive/`.
...
hermes curator backup         # take a manual snapshot of ~/.maia/skills/
...
Before every real curator pass, Hermes takes a tar.gz snapshot of `~/.maia/skills/` at `~/.maia/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:
...
- `~/.maia/skills/.bundled_manifest` (skills copied from the repo on install), and
- `~/.maia/skills/.hub/lock.json` (skills installed via `hermes skills install`).

Everything else in `~/.maia/skills/` is fair game for the curator. This includes:
...
The flag is stored as `"pinned": true` on the skill's entry in `~/.maia/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.maia/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.maia/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.maia/logs/curator/`:
...
This moves the skill back from `~/.maia/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.maia/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
...
- [Issue #7816](https://github.com/and270/maia/issues/7816) — original proposal and design discussion
