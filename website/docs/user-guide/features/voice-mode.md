...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Maia — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Maia supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.
...
1. **Maia installed** — `pip install maia` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `hermes model` or set your preferred provider credentials in `~/.maia/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.maia/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.maia/.env` manually for API keys.
:::
...
Voice mode is available in both the **classic CLI** (`hermes chat`) and the **TUI** (`hermes --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.maia/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.maia/config.yaml` (default: `ctrl+b`).
:::
...
:::tip
To disable the mention requirement in server channels, add to `~/.maia/.env`:
```bash
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.maia/.env`
...
- Review gateway logs: `tail -f ~/.maia/logs/gateway.log`
