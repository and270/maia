...
description: "Build themes and plugins for the Coorporate Hermes web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Coorporate Hermes web dashboard (`coorporate dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
…or restart `coorporate dashboard`.
...
2. Restart `coorporate dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
