...
description: "Build themes and plugins for the Maia web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Maia web dashboard (`maia dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.maia/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.maia/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.maia/dashboard-themes
```

```yaml
# ~/.maia/dashboard-themes/neon.yaml
name: neon
label: Neon
...
```yaml
# ~/.maia/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
```bash
mkdir -p ~/.maia/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.maia/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.maia/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
```json
// ~/.maia/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.maia/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.maia/plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.maia/plugins/<name>/dashboard/` | `user` |
...
cp plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.maia/dashboard-themes/

# Plugin
cp -r plugins/strike-freedom-cockpit ~/.maia/plugins/
```
...
Check that the file is in `~/.maia/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.maia/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.maia/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `maia dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.maia/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.maia/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
