...
exec(open(os.path.expanduser(
    "~/.maia/skills/red-teaming/godmode/scripts/load_godmode.py"
)).read())
...
The godmode skill integrates with two Maia config mechanisms:

### Ephemeral System Prompt (`config.yaml`)

Set the jailbreak system prompt in `~/.maia/config.yaml`:
...
Create `~/.maia/prefill.json` and reference it in config:
...
7. **execute_code sandbox lacks env vars** — Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.maia/.env"))`
