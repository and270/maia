...
sidebar_position: 5
title: "WhatsApp"
description: "Set up Maia as a WhatsApp bot via the built-in Baileys bridge"
---
...
The Baileys bridge saves its session under `~/.maia/platforms/whatsapp/session`. This means:
...
- Agent responses are prefixed with "⚕ **Maia**" by default. You can customize or disable this in `config.yaml`:
...
| **Session not persisting** | Check that `~/.maia/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.maia/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.maia/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.maia/platforms/whatsapp/session`
