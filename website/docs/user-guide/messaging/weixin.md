...
sidebar_position: 15
title: "Weixin (WeChat)"
description: "Connect Maia to personal WeChat accounts via the iLink Bot API"
---
...
5. Save the account credentials automatically to `~/.maia/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.maia/.env`:
...
- Tokens are saved per account+peer to `~/.maia/weixin/accounts/<account_id>.context-tokens.json`
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install maia[messaging]`. Alternatively, open the URL printed above the QR |
