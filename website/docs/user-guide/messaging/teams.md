...
description: "Set up Maia as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Maia to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Alternatively, configure via `~/.maia/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.maia/.env` |
