...
sidebar_position: 9
title: "Matrix"
description: "Set up Maia as a Matrix bot"
---

# Matrix Setup

Maia integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Maia pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).
...
Maia uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Optional behavior settings in `~/.maia/config.yaml`:
...
- Stores encryption keys in `~/.maia/platforms/matrix/store/` (legacy installs: `~/.maia/matrix/store/`)
...
If you delete `~/.maia/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.
...
2. Delete the local crypto store and restart Hermes:
   ```bash
   rm -f ~/.maia/platforms/matrix/store/crypto.db*
   # restart hermes
   ```
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   curl -X POST https://your-server/_matrix/client/v3/login \
     ...
     -d '{
       ...
       "password": "***",
       "initial_device_display_name": "Maia"
     }'
   ```

   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.maia/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.maia/platforms/matrix/store/crypto.db
   rm -f ~/.maia/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.maia/.env`:
...
**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.maia/.env` and restart the gateway. Use the full `@user:server` format.
