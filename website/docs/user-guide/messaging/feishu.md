...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Maia as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Maia integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.
...
Rules live in `~/.maia/feishu_comment_rules.json` (pairing grants in `~/.maia/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.maia/feishu_seen_message_ids.json`.
