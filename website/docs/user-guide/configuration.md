...
sidebar_position: 2
title: "Configuration"
description: "Configure Maia — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.maia/` directory for easy access.
...
2. **`~/.maia/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.maia/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Leaving these unset keeps the legacy defaults (`HERMES_API_TIMEOUT=1800`s, `HERMES_API_CALL_STALE_TIMEOUT=300`s, native Anthropic 900s). Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/and270/maia/blob/main/cli-config.yaml.example).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.maia/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.maia/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.maia/` (OAuth tokens, etc.) and synced before each command.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/hermes-agent` (HPC convention) → `~/.maia/sandboxes/singularity`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.maia/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.maia/.env` if it was saved with `hermes config set`.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Maia — Auxiliary Models Tutorial"
...
Or via environment variable (in `~/.maia/.env`):
...
:::note
Streaming is disabled by default. Enable it in `~/.maia/config.yaml` to try the streaming UX.
:::
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.maia/hermes-agent && git pull && pip install -e .
  gpu:
    type: exec
...
**Exa:** Set `EXA_API_KEY` in `~/.maia/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.maia/browser_recordings/
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.maia/SOUL.md` or `$MAIA_HOME/SOUL.md` |
...
```bash
# In ~/.maia/.env or ~/.maia/config.yaml:
