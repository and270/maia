...
import type { FrameEvent } from '@maia/ink'
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `maia-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`maia-tui ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    resetTerminalModes()
    process.stderr.write(`maia-tui: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write('maia-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
