...
- **Filesystem access:** Governance is always active. Folder policies are enforced by file tools, and gateway terminal/`execute_code` sessions receive only policy-derived Docker mounts. Unmatched paths are always denied; an assigned role alone grants no file. Subagents inherit the same sandbox, and isolation failure never falls back to host execution.
