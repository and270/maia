...
def cronjob(
    ...
    try:
        ...
        if normalized == "update":
            ...
            # only when this update supplies them. A job persisted before this
            # guard (or written directly to the store) may already hold an
            # unsafe named-provider + off-host base_url pair; if we only checked
            # when the update touches those axes, editing any unrelated field
            # would leave that exfil-capable pair active and schedulable. The
            # effective pair merges this update's normalized values over the
            # stored job, so an operator can remediate in the same update.
            eff_provider = (
