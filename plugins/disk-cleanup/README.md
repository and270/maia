...
Scoped strictly to `$MAIA_HOME` and `/tmp/hermes-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MAIA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MAIA_HOME` or `/tmp/hermes-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MAIA_HOME/disk-cleanup/` is itself excluded
- `$MAIA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
