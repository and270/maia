...
```bash
mkdir -p ~/.maia/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.maia/config.yaml
touch ~/.maia/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.maia/.env
```
...
│
├── skills/                   # Bundled skills (copied to ~/.maia/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.maia/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.maia/`)

| Path | Purpose |
|------|---------|
| `~/.maia/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.maia/.env` | API keys and secrets |
| `~/.maia/auth.json` | OAuth credentials (Nous Portal) |
| `~/.maia/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.maia/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.maia/state.db` | SQLite session database |
| `~/.maia/sessions/` | JSON session logs |
| `~/.maia/cron/` | Scheduled job data |
| `~/.maia/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. JSON logs go to `~/.maia/sessions/`.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `hermes setup` or update `~/.maia/.env` locally.
