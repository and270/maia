...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``maia-ink`` workspace package is
    ...
    assert "ui-tui/packages/maia-ink/" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/maia-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled maia-ink workspace package "
        "so ``await import('@maia/ink')`` resolves at runtime."
    )
