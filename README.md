...
- A **Write after approval** grant means the requester already has conditional write access. Maia stages the exact edit, tags eligible approvers, and leaves the original unchanged. An eligible approver can use the card or reply `approve` / `aprovo` in the same conversation when the request is unambiguous; the decision applies only that staged edit and never changes file permissions.
