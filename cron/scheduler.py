...
def _guard_job_credential_exfil(job: dict) -> None:
    ...
    credential (runtime backstop; CWE-200/CWE-522).
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        # would ship a named provider's stored credential to an off-host
        # endpoint (CWE-200/CWE-522). The cron tool validates this on
        # create/update, but a job persisted before that guard — or written
        # directly to the jobs store — reaches this sink unchecked. Fail closed
        # before resolution so no off-host call is ever made with a stored key.
