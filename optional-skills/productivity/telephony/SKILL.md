...
- save provider credentials into `~/.maia/.env`
...
```bash
SCRIPT="$(find ~/.maia/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
3. Buy it and save it into `~/.maia/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
3. persist that number to `~/.maia/.env`
