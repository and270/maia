...
3. **Configure profiles** — patch each profile's
   `~/.maia/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
...
Each profile has a YAML config at `~/.maia/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.maia/profiles/{profile}/config.yaml")
with open(p) as f:
...
Each profile gets a `SOUL.md` at `~/.maia/profiles/<name>/SOUL.md` that
...
`~/.maia/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.maia/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.maia/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.maia/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}
