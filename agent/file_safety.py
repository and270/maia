...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            # Maia keeps shell startup files write-denied (upstream relaxed
            # this in 81e42335a for single-user installs): in a multi-user
            # corporate gateway a prompt-injected write to .bashrc runs code
...
def is_write_denied_static(path: str) -> bool:
    ...
    # Maia control-plane files: block both the ACTIVE profile's view
    # (hermes_home) AND the global root view. config.yaml carries the
    # governance config (roles, folder policies) — a prompt-injected
    # write_file must not be able to rewrite it. Upstream relaxed this in
    # 81e42335a for single-user installs; Maia keeps the hard block.
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    Maia governance folder policies are checked first; then three
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    """
    # Maia governance: folder read policies from governance config apply
    # before the built-in denylist.
    try:
